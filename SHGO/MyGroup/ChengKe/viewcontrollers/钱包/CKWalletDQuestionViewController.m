//
//  CKWalletDQuestionViewController.m
//  SHGO
//
...
@interface CKWalletDQuestionViewController ()

@end

@implementation CKWalletDQuestionViewController
...
- (void)viewDidLoad {
    ...
    UIView *myView = [[UIView alloc] initWithFrame:CGRectMake(20*PROPORTION750, 30*PROPORTION750, AL_DEVICE_WIDTH-40*PROPORTION750, 360*PROPORTION750)];
    ...
    q1LB.attributedText = [self stringForQuestion:YES inputStr:@"Q 什么是余额？"];
    q1LB.textAlignment = NSTextAlignmentLeft;
    [myView addSubview:q1LB];
    
    UILabel *a1LB = [[UILabel alloc] initWithFrame:CGRectMake(50*PROPORTION750, q1LB.bottom+30*PROPORTION750, myView.width-100*PROPORTION750, 70*PROPORTION750)];
    a1LB.attributedText = [self stringForQuestion:NO inputStr:@"A 余额是您在使用小马出行中可以使用的账户资金，支付车费时会优先使用。"];
    a1LB.numberOfLines = 2;
    a1LB.textAlignment = NSTextAlignmentLeft;
    a1LB.font = SYSF750(25);
    [myView addSubview:a1LB];
    
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, 198*PROPORTION750, myView.width, 2*PROPORTION750)];
    ...
    q2LB.attributedText = [self stringForQuestion:YES inputStr:@"Q 余额如何使用？"];
    ...
    a2LB.attributedText = [self stringForQuestion:NO inputStr:@"A 在支付车费时，将优先使用余额进行支付。"];
